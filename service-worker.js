 ...
 */
const CACHE_VERSION = 'expense-tracker-shell-20260619-36';
...
// Optional assets: cached best-effort. A missing file here must NOT fail install
// (addAll is atomic and would reject the whole batch), so they are added separately.
    ...
    caches.open(CACHE_VERSION)
      .then((cache) => cache.addAll(PRECACHE)
        .then(() => Promise.allSettled(
          OPTIONAL_FILES.map((f) => cache.add(f).catch(() => null))
        ))
      )
