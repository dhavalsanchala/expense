/* ============================================================
    ...
    // Color coding for expense type badges — a heat-map temperature scale rather
    // than per-category hues, built from the brand palette (teal-blue, indigo,
    // green, violet, magenta, red-orange). Cold = money that grows; neutral = money
    // that just moves; hot = money that's spent. bg/fg are light-mode; dbg/dfg
    // dark-mode; accent/daccent are the 3px row bar.
    const TYPE_STYLES = {
      'Earning':        { bg: '#E0F0F5', fg: '#005979', dbg: '#00212D', dfg: '#9ECEE0', accent: '#007FAD', daccent: '#4CA5C6' },
      'Saving':         { bg: '#E5E0EA', fg: '#180035', dbg: '#1A0E29', dfg: '#AB9EBB', accent: '#23004C', daccent: '#7E5FA8' },
      'Investment':     { bg: '#E5F0E0', fg: '#1B5D00', dbg: '#0A2300', dfg: '#ADD19E', accent: '#268500', daccent: '#67AA4C' },
      'Transfer':       { bg: '#EFE0FC', fg: '#5500A1', dbg: '#20003C', dfg: '#CC9EF6', accent: '#7A00E6', daccent: '#A24CEE' },
      'Essential':      { bg: '#FAE3F7', fg: '#950F82', dbg: '#370530', dfg: '#EFA6E4', accent: '#D515B9', daccent: '#E25BCE' },
      'Non-essential':  { bg: '#FBE7E3', fg: '#99290F', dbg: '#390F06', dfg: '#F1B4A6', accent: '#DA3A16', daccent: '#E5755C' },
      'Vacation':       { bg: '#F8E4E2', fg: '#8C1507', dbg: '#340803', dfg: '#EAAAA2', accent: '#C81E0A', daccent: '#D86254' }
    };
    // Fallback palette for custom user-added types (assigned by stable hash).
    // Off the temperature scale so custom types read as "uncategorized".
    const TYPE_FALLBACK = [
      { bg: '#EFE0FC', fg: '#5500A1', dbg: '#20003C', dfg: '#CC9EF6', accent: '#7A00E6', daccent: '#A24CEE' },
      { bg: '#FAE3F7', fg: '#950F82', dbg: '#370530', dfg: '#EFA6E4', accent: '#D515B9', daccent: '#E25BCE' }
    ];
    ...
    class ExpenseTrackerApp {
      ...
      onAddTabClick() {
        this.resetForm();
        this.setTab('add');
        // Auto-focus the amount field so the numeric keyboard opens immediately.
        // Deferred so the section is visible before focusing (required on mobile).
        const txAmount = document.getElementById('txAmount');
        if (txAmount) {
          requestAnimationFrame(() => {
            txAmount.focus({ preventScroll: false });
          });
        }
      }
      ...
      showHelp() {
        const info = this._cardInfo();
        const order = ['monthlyBudget','spent','budget','cashflow','byType','tracked','events','goals','accounts','reportFilters','export'];
        const titleEl = document.getElementById('cardInfoTitle');
        const bodyEl = document.getElementById('cardInfoBody');
        if (titleEl) titleEl.textContent = 'Help';
        if (bodyEl) {
          bodyEl.innerHTML = order.map(k => {
            const e = info[k];
            return `<div style="margin-bottom:14px;"><div style="font-weight:600;color:var(--text);margin-bottom:3px;">${this.esc(e[0])}</div><div style="color:var(--text-secondary);">${this.esc(e[1])}</div></div>`;
          }).join('');
        }
        this.openModal('cardInfoModal');
      }
      ...
      renderDashboard() {
        ...
        if (sub) {
          sub.textContent = remaining >= 0 ? this.fmt(remaining) : '-' + this.fmt(Math.abs(remaining));
        }
        const dashBudgetCard = document.getElementById('dashBudgetCard');
        if (dashBudgetCard) dashBudgetCard.className = 'hero-stat ' + (remaining < 0 ? 'danger' : remaining < budget * 0.2 ? 'warning' : '');
      ...
      editTransaction(id) {
        ...
        // If not manually locked, reflect the live date-driven status on the toggle.
        this.applyAutoStatusToForm();
        this.syncTxStatusUI();
      ...
      resetForm() {
        ...
        if (form) form.reset();
        // txItem and txDate live OUTSIDE #txForm, so form.reset() does not clear
        // them. Clear the item field (and its autocomplete) explicitly here.
        const txItem = document.getElementById('txItem');
        if (txItem) txItem.value = '';
        const acItem = document.getElementById('ac-item');
        if (acItem) { acItem.innerHTML = ''; acItem.style.display = 'none'; }
        const fields = [
          ['txId', ''], ['txAmount', ''], ['txType', 'Earning'], ['txFrequency', 'Variable'],
      ...
      syncHistoryChips() {
        ...
        const chips = document.querySelectorAll('#historyQuickFilters .chip');
        chips.forEach(chip => {
      ...
      renderHistory() {
        ...
        if (paginated.length === 0) {
          list.innerHTML = '<div class="empty" style="padding:32px 16px;"><div class="empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" style="width:40px;height:40px;"><path d="M3 7l1.5 9.5A2 2 0 0 0 6.5 18h11a2 2 0 0 0 2-1.5L21 7"/><path d="M3 7l2-3h14l2 3"/><path d="M3 7h18"/><path d="M9.5 11h5"/></svg></div><p>No transactions found</p></div>';
        } else {
          let lastDate = '';
          list.innerHTML = paginated.map((t) => {
            ...
            <div class="tx-item tx-history-row${selected ? ' tx-selected' : ''}"
              ...
              <div class="tx-content" style="min-width:0;">
                ...
                <div class="tx-meta" style="font-size:10px;">${this.esc(t.category || '')}${t.mode ? ' · ' + this.esc(t.mode) : ''} · ${t.date}${t.splitGroup ? ' · split' : ''}${this.historySelectMode ? '' : `${!t.mirrorOf ? `<button class="tx-move-btn" data-id="${t.id}" title="Move to other profile" aria-label="Move to other profile"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M17 3l4 4-4 4"/><path d="M21 7H8"/><path d="M7 21l-4-4 4-4"/><path d="M3 17h13"/></svg></button>` : ''}<button class="tx-x-btn" data-id="${t.id}" title="Delete" aria-label="Delete transaction">&times;</button>`}</div>
              </div>
      ...
      bindHistoryRowEvents() {
        ...
        rows.forEach(row => {
          ...
          row.addEventListener('click', (e) => {
            ...
            if (e.target.closest('.tx-move-btn')) return; // handled separately
        ...
        list.querySelectorAll('.tx-move-btn').forEach(btn => {
          btn.addEventListener('click', (e) => {
            e.stopPropagation();
            this.moveTransactionToProfile(btn.getAttribute('data-id'));
          });
        });
      }
      ...
      /** Relocate an existing expense's ledger entry to the other profile
       *  (self <-> joint). Removes it (and any split children + transfer mirror)
       *  from the current profile and inserts a clean copy into the other one. */
      moveTransactionToProfile(id) {
        const t = this.data.transactions.find(x => x && x.id === id);
        if (!t) return;
        if (t.mirrorOf) {
          this.toast('This is a synced mirror entry. Move the original instead.', 'err');
          return;
        }
        const isSelf = this.allData.activeProfile === 'self';
        const targetProfile = isSelf ? 'wife' : 'self';
        const targetData = this.allData.profiles[targetProfile];
        if (!targetData || !Array.isArray(targetData.transactions)) {
          this.toast('Other profile is unavailable', 'err');
          return;
        }
        const targetLabel = isSelf ? 'Joint W/ Spouse' : 'Self';
        const targetSource = isSelf ? 'Joint Account' : 'Personal';

        this.confirmCallback = () => {
          this._doMoveTransactionToProfile(id, targetProfile, targetSource);
          this.closeModal('confirmModal');
        };
        const confirmTitle = document.getElementById('confirmTitle');
        const confirmBody = document.getElementById('confirmBody');
        const confirmBtn = document.getElementById('confirmBtn');
        if (confirmTitle) confirmTitle.textContent = 'Move to Other Profile?';
        if (confirmBody) confirmBody.textContent = `Move "${t.item || 'Untitled'}" (${this.fmt(this.txDisplayAmount(t))}) to ${targetLabel}? It will be removed from the current profile.`;
        if (confirmBtn) {
      ...
      _doMoveTransactionToProfile(id, targetProfile, targetSource) {
        this._nwCache = null;
        this.pushUndo('Move transaction to profile');
        const t = this.data.transactions.find(x => x && x.id === id);
        if (!t) return;
        const targetData = this.allData.profiles[targetProfile];
        if (!targetData || !Array.isArray(targetData.transactions)) return;

        // Gather split children before we delete anything.
        const children = t.isSplitParent
          ? this.data.transactions.filter(x => x && x.splitGroup === t.id)
          : [];

        // If this was a cross-profile transfer, scrub its mirror from all profiles.
        if (!t.mirrorOf && t.type === 'Transfer') {
          Object.values(this.allData.profiles).forEach(p => {
            if (p && Array.isArray(p.transactions)) {
              p.transactions = p.transactions.filter(x => x.mirrorOf !== t.id);
            }
          });
        }

        // Build the relocated parent. Strip series/mirror linkage — the moved entry
        // becomes a standalone item in the target ledger and is re-tagged with the
        // target profile's source.
        const newId = 'tx_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7);
        const { recurringId, mirrorOf, seriesIndex, ...rest } = t;
        const movedParent = { ...rest, id: newId, source: targetSource };
        targetData.transactions.push(movedParent);

        // Relocate split children, re-pointing them at the new parent id.
        children.forEach(child => {
          const { recurringId: cRec, mirrorOf: cMir, seriesIndex: cSi, ...cRest } = child;
          targetData.transactions.push({
            ...cRest,
            id: 'tx_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7),
            splitGroup: newId,
            source: targetSource
        ...
        // Remove the original (and its children) from the current profile.
        const removeIds = new Set([t.id, ...children.map(c => c.id)]);
        this.data.transactions = this.data.transactions.filter(x => x && !removeIds.has(x.id) && x.splitGroup !== t.id);

        // Re-sync the mirror in the target profile if the moved entry is itself a
        // cross-profile transfer (its counterpart now lives where it was moved from).
        if (movedParent.type === 'Transfer' && ['Personal to Joint', 'Joint to Personal'].includes(movedParent.category)) {
          const savedActive = this.allData.activeProfile;
          this.allData.activeProfile = targetProfile;
          this.data = targetData;
          this.syncTransferMirror(movedParent);
          this.allData.activeProfile = savedActive;
          this.data = this.allData.profiles[savedActive];
        }

        this.save();
        const targetLabel = targetProfile === 'self' ? 'Self' : 'Joint W/ Spouse';
        // Stay where the user invoked the move from: history stays on history,
        // the edit screen returns home with a fresh form.
        ...
        this.toast(`Moved to ${targetLabel}`, 'ok');
      }
      ...
      openHistoryFilters() {
        this.openModal('historyFilterModal');
      }

      resetHistoryFilters() {
        const wrap = document.getElementById('historyQuickFilters');
        if (wrap) wrap.querySelectorAll('.chip.on').forEach(b => b.classList.remove('on'));
        const historyDateFrom = document.getElementById('historyDateFrom');
        const historyDateTo = document.getElementById('historyDateTo');
      ...
      // Count active filters and reflect it on the History "Filters" button.
      updateHistoryFilterBadge() {
        let count = 0;
        const from = document.getElementById('historyDateFrom');
        const to = document.getElementById('historyDateTo');
        if ((from && from.value) || (to && to.value)) count++;
        if (this.historyFilterType && this.historyFilterType !== 'all') count++;
        if (this.historySourceFilter) count++;
        if (this.historyEventId) count++;
        const label = document.getElementById('historyFilterBtnLabel');
        const btn = document.getElementById('historyFilterBtn');
        if (label) label.textContent = count > 0 ? `Filters · ${count}` : 'Filters';
        if (btn) btn.classList.toggle('btn-primary', count > 0);
      }
      ...
      // Clear a manual status override and return the item to date-driven auto status.
      clearTxStatusOverride() {
        const txStatus = document.getElementById('txStatus');
        if (txStatus) txStatus.dataset.userSet = '';
        this.applyAutoStatusToForm();
        this.syncTxStatusUI();
      ...
      // Compute the auto status from the dates currently in the form and reflect it
      // on the toggle (only when the user hasn't manually overridden).
      applyAutoStatusToForm() {
        const txStatus = document.getElementById('txStatus');
        if (!txStatus || txStatus.dataset.userSet === 'true') return;
        const startDate = document.getElementById('txStartDate');
        const endDate = document.getElementById('txEndDate');
        const auto = this.getTxStatus({
        ...
        txStatus.value = auto;
        document.querySelectorAll('#txStatusToggle .type-btn').forEach(b => {
          b.classList.toggle('on', b.textContent.trim() === auto);
        });
      }

      // Toggle the "Auto" tag / "Reset to Auto" affordance based on override state.
      syncTxStatusUI() {
        const txStatus = document.getElementById('txStatus');
        const isManual = txStatus && txStatus.dataset.userSet === 'true';
        const autoTag = document.getElementById('txStatusAutoTag');
        const resetBtn = document.getElementById('txStatusResetBtn');
        if (autoTag) autoTag.style.display = isManual ? 'none' : 'inline';
        if (resetBtn) resetBtn.style.display = isManual ? 'inline' : 'none';
      }
      ...
      /**
       * Auto-assign lifecycle status from start/end dates, unless the user has
       * manually locked it (statusLocked) — in which case the manual choice wins.
       *   today <  startDate            -> Planned
       *   startDate <= today <= endDate -> In-Progress
       *   today >  endDate              -> Completed
       * Overdue is never auto-assigned; it is a manual-only status.
       */
      getTxStatus(t) {
        const today = getLocalDateStr();
        const manualStatus = t.status || 'Planned';
        // Manual choice always wins (covers Overdue and any deliberate override).
        if (t.statusLocked) return manualStatus;
        // Without both dates we can't derive anything; fall back to whatever's stored.
        if (!t.startDate || !t.endDate) return manualStatus;
        if (today < t.startDate) return 'Planned';
      ...
      renderTrack() {
        ...
        if (txs.length === 0) {
          container.innerHTML = '<div class="empty" style="padding:32px 16px;"><div class="empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" style="width:40px;height:40px;"><path d="M3 7l1.5 9.5A2 2 0 0 0 6.5 18h11a2 2 0 0 0 2-1.5L21 7"/><path d="M3 7l2-3h14l2 3"/><path d="M3 7h18"/><path d="M9.5 11h5"/></svg></div><p>No tracked products. Enable "Product Lifecycle" when adding an expense.</p></div>';
          return;
        }
      ...
      renderReportSummary(txs) {
        ...
        if (txs.length === 0) {
          summary.innerHTML = '';
          container.innerHTML = '<div class="card" style="text-align:center;padding:32px 16px;"><div class="empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" style="width:40px;height:40px;"><path d="M3 7l1.5 9.5A2 2 0 0 0 6.5 18h11a2 2 0 0 0 2-1.5L21 7"/><path d="M3 7l2-3h14l2 3"/><path d="M3 7h18"/><path d="M9.5 11h5"/></svg></div><p style="color:var(--text-tertiary);font-size:14px;font-weight:500;">No transactions in selected range</p></div>';
          return;
        }
      ...
      renderReportEvent(txs) {
        ...
        if (txs.length === 0 || events.length === 0) {
          summary.innerHTML = '';
          container.innerHTML = '<div class="card" style="text-align:center;padding:32px 16px;"><div class="empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" style="width:40px;height:40px;"><path d="M3 7l1.5 9.5A2 2 0 0 0 6.5 18h11a2 2 0 0 0 2-1.5L21 7"/><path d="M3 7l2-3h14l2 3"/><path d="M3 7h18"/><path d="M9.5 11h5"/></svg></div><p style="color:var(--text-tertiary);font-size:14px;font-weight:500;">No event data in selected range</p></div>';
          return;
        }
        const evtMap = {};
        txs.forEach(t => {
      ...
      printReport() {
        ...
        printWindow.document.write(`
          ...
          <style>
            ...
            .green{color:#268500}.red{color:#DA3A16}
      ...
      exportReportHTML() {
        ...
        const gridRows = gridTypes.map(ty => {
          ...
          const barColor = (isExp && pct > 100) ? '#DA3A16' : (isExp && pct > 90) ? '#C9943A' : typeStyle(ty).accent;
...
<style>
  *{box-sizing:border-box} body{font-family:Inter,-apple-system,Segoe UI,Roboto,sans-serif;margin:0;background:#f5f5f5;color:#1a1a1a;padding:28px}
  ...
  .value{font-size:22px;font-weight:700} .green{color:#268500}.red{color:#DA3A16}.muted{color:#6b6b6b;font-weight:500;font-size:12px}
  ...
  <footer>Generated by Expense Tracker</footer>
      ...
      renderEvents() {
        ...
        const events = this.data.events || [];
        if (events.length === 0) {
          if (container) container.innerHTML = '<div class="empty" style="padding:32px 16px;"><div class="empty-icon"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round" style="width:40px;height:40px;"><path d="M3 7l1.5 9.5A2 2 0 0 0 6.5 18h11a2 2 0 0 0 2-1.5L21 7"/><path d="M3 7l2-3h14l2 3"/><path d="M3 7h18"/><path d="M9.5 11h5"/></svg></div><p>No events yet. Tap above to create one.</p></div>';
          return;
        }
        const today = getLocalDateStr();
        if (container) {
      ...
      getBackupKey(profileId) {
        return `expenseTracker_backups_${profileId || this.allData.activeProfile}`;
      }

      /** Read the backup list for any profile (defaults to active). */
      getBackupsFor(profileId) {
        try {
          const raw = localStorage.getItem(this.getBackupKey(profileId));
          return raw ? JSON.parse(raw) : [];
        } catch (e) { return []; }
      }

      /** Run at most once per calendar day. Snapshots EVERY profile (self + joint)
       *  into its own per-profile backup store, tagged as an auto-backup so manual
       *  and automatic backups can be told apart. Keeps the newest 10 per profile. */
      runDailyAutoBackup() {
        try {
          const today = getLocalDateStr();
          const profiles = this.allData && this.allData.profiles ? this.allData.profiles : {};
          Object.keys(profiles).forEach((pid) => {
            const pdata = profiles[pid];
            if (!pdata || !Array.isArray(pdata.transactions)) return;
            const backups = this.getBackupsFor(pid);
            // Skip if an auto-backup already exists for today on this profile.
            const hasToday = backups.some(b => b && b.auto && b.day === today);
            if (hasToday) return;
            ...
            localStorage.setItem(this.getBackupKey(pid), JSON.stringify(backups));
          });
        } catch (e) {
          // Never let a backup failure break app start-up.
          console.error('[AUTO-BACKUP]', e && e.message);
        }
      }
      ...
      getBackups() {
        return this.getBackupsFor(this.allData.activeProfile);
      }
        ...
        container.innerHTML = backups.map((b, i) => `
          <div class="backup-row">
            <div class="backup-info">
              <div class="backup-name">${this.esc(b.name)}${b.auto ? ' <span style="font-size:8px;background:var(--success-soft);color:var(--success);padding:1px 4px;border-radius:3px;font-weight:700;vertical-align:middle;">AUTO</span>' : ''}</div>
